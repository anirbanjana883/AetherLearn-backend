...
## 6. System Flow: Chunked Upload Pipeline
```
...
## 8. Fault Tolerance
- MongoDB Transactions: Wrapping lecture/section creation in `session.startTransaction()`. If the server crashes after creating a lecture but before updating the section array, the transaction rolls back. No "ghost" lectures are created.
...
- Direct-to-S3 Pre-signed URLs: Bypassing the Node.js server entirely by generating AWS S3 M